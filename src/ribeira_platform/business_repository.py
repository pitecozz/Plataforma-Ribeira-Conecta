...
class BusinessRepository:
    ...
    def list_assets_for_property(self, tenant_id: str, property_id: str) -> list[Asset]:
        ...
        The caller establishes the tenant transaction/RLS context. This method
        deliberately returns persisted source/classification/context fields and
        never fills absent coordinates or operational facts.
        """
        p = self.placeholder
        geometry_column = (
        ...
        rows = self._execute(
            f"""SELECT id,tenant_id,asset_type,name,serial_number,status,property_id,site_id,
                 ...
                 FROM asset
                WHERE tenant_id={p} AND property_id={p}
                ORDER BY name ASC,id ASC""",  # nosec B608 - identifiers are fixed
