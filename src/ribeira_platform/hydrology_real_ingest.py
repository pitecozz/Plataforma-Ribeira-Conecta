...
from .vale_public_data import InmetWis2HttpProvider, SidraPamProvider, parse_sidra_baselines


def main() -> None:
    parser = argparse.ArgumentParser(description="Bounded official Vale public-data ingestion")
        ...
        with store.transaction():
            ...
            for record in records:
                ...
                if record.raw_value is not None:
                    store.record_wis2_rain(record, station_id=station_id, fetched_at=fetched_at)
            station_total, observation_total = store.wis2_ingestion_counts()
            store.upsert_source_health(
                "INMET_WIS2_HTTP", "AVAILABLE", last_attempt=fetched_at.isoformat(),
            ...
            store.upsert_source_health(
                "INMET_PORTAL", "DEGRADED", last_attempt=fetched_at.isoformat(),
                failure_code="HTTP_RESET", failure_detail_sanitized="official portal channel reset during bounded diagnostic",
            )
            store.upsert_source_health(
                "INMET_WIS2_MQTT", "AVAILABLE", last_attempt=fetched_at.isoformat(),
                failure_code="NO_NOTIFICATION_IN_WINDOW", failure_detail_sanitized="bounded outbound MQTT probe received no notification; HTTP OGC API remains available",
            )
            ...
            if not small:
                raise RuntimeError("SIDRA small official query returned no municipality rows")
            for code, values in small.items():
                store.upsert_banana_baseline(
                    municipality_code=code, values=values, metadata=metadata,
            ...
            store.upsert_source_health(
                "IBGE_PAM_SIDRA", "AVAILABLE", last_attempt=fetched_at.isoformat(),
                last_success=fetched_at.isoformat(),
            )
        for key, value in {
