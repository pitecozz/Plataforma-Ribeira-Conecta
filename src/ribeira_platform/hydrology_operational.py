...
def reconciliation_window(
    *, now: datetime, historical: bool, lookback_hours: int
...
def _run(
    *, historical: bool, lookback_hours: int, max_pages: int, max_records: int
) -> dict[str, int | str]:
    dsn = os.getenv("RIBEIRA_DATABASE_URL")
    if not dsn:
        raise RuntimeError("RIBEIRA_DATABASE_URL is required")
    started = datetime.now(timezone.utc)
    begin, end = reconciliation_window(
        now=started, historical=historical, lookback_hours=lookback_hours
    )
    context = "HISTORICAL_BACKFILL" if historical else "OPERATIONAL_RECONCILIATION"
    store = PostgresStore(dsn)
    received = target_candidates = inserted = duplicates = ignored = invalid = 0
        ...
        with store.transaction():
            ...
            try:
                ...
                store.upsert_source_health(
                    "INMET_WIS2_HTTP",
                    "DEGRADED",
                    last_attempt=finished.isoformat(),
                    latency_seconds=(finished - started).total_seconds(),
                    failure_code=type(exc).__name__,
                    failure_detail_sanitized="bounded official WIS2 request failed",
                )
                store.record_hydro_ingestion_run(
                    ...
                    invalid=0,
                    errors=1,
                    max_report_lag_seconds=None,
                    detail="bounded official WIS2 request failed",
                )
            ...
            for item in records:
                ...
                if relation == "OUTSIDE_SCOPE":
                    ignored += 1
                    reasons["IGNORED_OUTSIDE_SPATIAL_SCOPE"] += 1
                    continue
                if item.raw_value is None:
                    ignored += 1
                    reasons["IGNORED_MISSING_SOURCE_VALUE"] += 1
                    continue
                target_candidates += 1
                try:
                    station = store.upsert_wis2_station(item, relation=relation)
                    if store.record_wis2_rain(
                        item, station_id=station, fetched_at=started
                    ):
                        inserted += 1
                    else:
                        duplicates += 1
                    ...
                    invalid += 1
                    reasons["REJECTED_VALIDATION_OR_PERSISTENCE"] += 1
            finished = datetime.now(timezone.utc)
            lag = max(
            ...
            store.record_hydro_ingestion_run(
                ...
                invalid=invalid,
                errors=0,
                max_report_lag_seconds=lag,
                detail="bbox " + ("accepted" if bbox_supported else "fallback"),
                quality_reason_counts=dict(reasons),
            )
            timeline = (
                store.rebuild_september_event_timeline(end_at=end) if historical else {}
            )
        return {
            "STATUS": "SUCCESS",
            "CONTEXT": context,
            ...
            "TIMELINE_ENTRIES": sum(timeline.values()),
        }
    finally:
        store.close()
...
def main() -> None:
    ...
    if (
        not 3 <= args.lookback_hours <= 24
        or not 1 <= args.max_pages <= 10
        or not 1 <= args.max_records <= 2000
    ):
        raise ValueError("unsafe reconciliation bounds")
    result = _run(
        historical=args.historical_backfill,
