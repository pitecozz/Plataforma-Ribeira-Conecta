...
| F-RS-001 | Sentinel scene catalogue, freshness and provenance | `PARTIAL` | Select reproducible scenes and retain temporal catalogue history for property refresh. | Official CDSE STAC item metadata, exact footprints, selection policy and durable job queue. | No Process API scene proof; latest available/usable/processed are distinct. Scheduled policy records remain the next increment. [Refresh](../geospatial/AUTOMATIC_PROPERTY_REFRESH.md). |
