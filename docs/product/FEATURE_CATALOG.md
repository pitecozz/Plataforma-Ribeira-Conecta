...
| F-RULE-001 | Scoped Rules, decisions, actions and outcomes | `PARTIAL` | Makes context yield a traceable, non-automated recommendation and closure. | Versioned rule, evidence, tenant/property/asset scope, action/result. | `ASSET` scope is selected only through an explicit property-linked asset evaluation; its registration evidence and asset ID remain in the decision, and property evaluation never inherits it. Customer/field/zone/crop inheritance remains pending. Farm360 permits explicit action-result capture only with `action:write`; original decisions remain immutable. Asset evaluation is an explicit `asset:read` + `decision:read` workflow that requires registration provenance and refreshes the decision history; it is never automatic. `engine.py`, `service.py`, migrations 029/031/036/037. |
...
| Scoped rule/action/outcome | `service.py`, rule payload, asset evaluation and outcome routes in `api.py` | migrations 029, 031, 036 and 037 | `test_vertical_slice.py`, [rule catalogue](../rules/BUSINESS_RULE_CATALOG.md) |
