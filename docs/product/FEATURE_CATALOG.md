...
| F-MAPS-005 | Non-legal field/talhão context | `FOUNDATION` | Tenant property field registration for future Farm360/Soil/Agro context. | Persisted property boundary; source/time; versioned WGS84 geometry. | `property:write` creates and `property:read` lists only tenant-contained geometries. It stores provenance/evidence/audit and makes no legal, crop, soil, or agronomic claim. Field rule scope and map editor are follow-on work. |
