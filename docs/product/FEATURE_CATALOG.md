...
| F-RULE-001 | Scoped Rules, decisions, actions and outcomes | `PARTIAL` | Makes context yield a traceable, non-automated recommendation and closure. | Versioned rule, evidence, tenant/property/asset scope, action/result. | `ASSET` scope is selected only through an explicit property-linked asset evaluation; its registration evidence and asset ID remain in the decision, and property evaluation never inherits it. Customer/field/zone/crop inheritance remains pending. Farm360 permits explicit action-result capture only with `action:write`; original decisions remain immutable. `engine.py`, `service.py`, migrations 029/031/036/037. |
