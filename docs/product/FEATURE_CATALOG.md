...
| F-SEC-002 | Pilot identity/onboarding/deployment path | `PARTIAL` | OIDC membership, temporary HTTPS edge and runbooks. | No public DB/metrics/dev ports; Quick Tunnel origin must be exact, with no OIDC wildcard. | `BLOCKED` pending operator Quick Tunnel origin, OIDC configuration and membership. |
