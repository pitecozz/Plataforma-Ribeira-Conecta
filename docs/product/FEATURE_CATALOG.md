...
| F-RULE-001 | Scoped Rules, decisions, actions and outcomes | `PARTIAL` | Makes context yield a traceable, non-automated recommendation and closure. | Versioned rule, evidence, tenant/property/asset/field/customer scope, action/result. | `ASSET` scope is selected only through an explicit property-linked asset evaluation; its registration evidence and asset ID remain in the decision, and property evaluation never inherits it. `FIELD` scope is selected only through explicit evaluation of a source-backed non-legal field/talhão; its `FIELD_REGISTRATION` evidence and field ID remain in the decision, and it never asserts crop, soil or diagnosis. `CUSTOMER` scope requires an active, tenant-local temporal customer-property link; the selected scope/customer are immutable decision and audit context. Equally specific rules return `CONFLICTING`, never insertion-order selection. Zone/crop inheritance remains pending. Farm360 permits explicit action-result capture only with `action:write`; original decisions remain immutable. Asset evaluation is an explicit `asset:read` + `decision:read` workflow; field evaluation is an explicit `property:read` + `decision:read` workflow. Neither is automatic. Provenance-valid `NDVI_DELTA` mean evaluation is an explicit `geospatial:read` + `decision:read` property workflow: it persists derived-product evidence and exact rule/version/scope, creates a non-automated targeted inspection recommendation only on trigger, and never asserts diagnosis. `engine.py`, `service.py`, migrations 029/031/036/037/038/041. |
