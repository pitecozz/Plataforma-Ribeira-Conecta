...
| F-RULE-001 | Scoped Rules, decisions, actions and outcomes | `PARTIAL` | Makes context yield a traceable, non-automated recommendation and closure. | Versioned rule, evidence, tenant/property/asset/customer scope, action/result. | `ASSET` scope is selected only through an explicit property-linked asset evaluation; its registration evidence and asset ID remain in the decision, and property evaluation never inherits it. `CUSTOMER` scope requires an active, tenant-local temporal customer-property link; the selected scope/customer are immutable decision and audit context. Equally specific rules return `CONFLICTING`, never insertion-order selection. Field/zone/crop inheritance remains pending. Farm360 permits explicit action-result capture only with `action:write`; original decisions remain immutable. Asset evaluation is an explicit `asset:read` + `decision:read` workflow that requires registration provenance and refreshes the decision history; it is never automatic. `engine.py`, `service.py`, migrations 029/031/036/037/038. |
| F-FIELD-001 | Non-legal field/talhão context | `FOUNDATION` | Registers a source-backed operational field within a property for later rules, Soil and Agro workflows. | Persisted property boundary, WGS84 field geometry, source, timestamp, PostGIS/RLS. | The geometry must be fully contained by the tenant property and records immutable checksum/version, `FIELD_REGISTRATION` evidence and audit. It is not title, survey, crop, soil or agronomic evidence; field rule scope is not active. Farm360 lists the inventory and displays its own map layer; `property:write` gates registration from the workspace. `field_context.py`, migrations 039/040, `test_field_context*.py`, `FieldPanel.test.tsx`. |
...
| Scoped rule/action/outcome | `service.py`, rule payload, asset evaluation and outcome routes in `api.py` | migrations 029, 031, 036, 037 and 038 | `test_vertical_slice.py`, PostgreSQL integration coverage, [rule catalogue](../rules/BUSINESS_RULE_CATALOG.md) |
...
| F-RULE-002 | Rule applicability: customer, field, zone, asset and crop | `PARTIAL` | Customer applicability is available through active temporal links; field, zone and crop require their own versioned context models. | Customer scope never infers a customer or selects an equally specific rule by order. | Field/zone/crop remain Rules P1 follow-on. |
