...
|---|---|---|---|
| Versioned decision rules | `IMPLEMENTED` | `TENANT`, `PROPERTY`, explicit `ASSET`, explicit `FIELD` and `CUSTOMER`. Asset and field rules are considered only by explicit evaluation context, require tenant-local source-backed context linked to the decision property, and never apply during property-only evaluation. Field decisions retain registration evidence and the immutable boundary version/checksum. `NDVI_DELTA` selects `FIELD` scope only when its job and product snapshot the same immutable field boundary and processing clipped both quality-masked inputs to it; invalid or tampered provenance fails closed. | `engine.py`, `service.py`, migrations 036/038/041/043/044. |
