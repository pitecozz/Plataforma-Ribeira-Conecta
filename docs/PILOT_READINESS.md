...
| Secure external access | API, PostgreSQL and metrics loopback-only | production static ingress allowlists `/api/v1/*` and API health only | local + public ingress smoke | `PARTIAL` | `CLOUDFLARE_QUICK_TUNNEL=PILOT_TEST_ONLY`; exact origin is protected runtime state. Private ingress remains healthy, but the operator-managed Quick Tunnel process was absent during the V0.2 deployment, so public retest is blocked. Metrics, OpenAPI/docs and Redoc remain non-public through the ingress policy; API/PostgreSQL/metrics listeners remain loopback-only. `https://app.ribeiraconecta.com.br` is deferred as `FUTURE_CUSTOM_DOMAIN`. See [onboarding](pilot/PILOT_ONBOARDING.md). |
