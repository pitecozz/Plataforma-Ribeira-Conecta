...
|---|---|---|---|---|---|
| Login and tenant authorization | OIDC/RBAC/RLS contracts | exact runtime public-origin OIDC configuration | unit/integration | `BLOCKED` | The protected runtime is still in development-auth mode. At assessment, its database contained zero tenants, zero properties and zero active memberships; an operator must supply verified onboarding facts before production OIDC and membership configuration. No wildcard OIDC trust is permitted. |
