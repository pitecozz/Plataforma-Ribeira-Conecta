...
|---|---:|---|
| `PILOT_READINESS_PERCENT` | 100% | The real Hamilton journey on the permanent origin is manually verified. A real feedback submission is persisted with the correct tenant/property/user association and matching audit event after the connection-level transaction-serialization fix. |
...
| Property boundary and Farm360 | property/boundary APIs | customer-readable map/workspace | unit + PostGIS + component regression + real Hamilton visual acceptance | `BETA` | A verified pilot operational-analysis boundary exists with explicit source, `MANUAL_CONFIRMED` classification and legal-boundary verification set to false. The real property boundary is visibly rendered at property scale; unavailable optional context does not suppress confirmed data. |
| Map | boundary and authenticated tiles | MapLibre property/asset layer and selection | frontend component + public smoke + real authenticated visual acceptance | `BETA` | OpenFreeMap, the bundled MapLibre worker, property overlay lifecycle, fitBounds, preview and recenter control are accepted in the real Hamilton session. Basemap rendering is visualization infrastructure, not analytical evidence. |
| Assets and Digital Twin | persisted asset inventory/API | customer-readable list/detail and synchronized spatial layer | backend/frontend/PostGIS/RLS | `BETA` | The verified initial pilot asset inventory is registered through the audited bootstrap. VIEWER has persisted `asset:read` only; write/manage permissions remain denied. Close positions use a display-only overlap group that exposes each persisted asset without changing coordinates. |
| Context/evidence | scenes/products/provenance | customer satellite catalogue and provenance panel | backend/frontend | `PARTIAL` | A persisted Sentinel-2 catalogue scene is visible with acquisition/cloud metadata while no derived product exists. Catalogue metadata is not presented as NDVI, agronomic observation or diagnosis. |
...
| Feedback | tenant-isolated feedback API and audit | Farm360 form with explicit submission/error state | backend/frontend/PostGIS | `READY` | The real resubmission is persisted with matching `PILOT_FEEDBACK_SUBMITTED` audit evidence, correct tenant/property/user association, no duplicate and no pending API transaction. It creates no operational conclusion; triage/listing workflow is later. |
| Pilot E2E | public-access smoke plus real-session journey | `RIBEIRA_PLAYWRIGHT_BASE_URL` selects the protected permanent origin | official Playwright container + real session | `READY` | HTTPS, public frontend/API health, unauthenticated 401, non-public metrics/docs/OpenAPI, and the Auth0 PKCE authorization handoff are proven against `https://app.ribeiraconecta.com.br`. The real Hamilton session passed Home → Farm360 → map/assets → report → durable feedback. |
...
5. Keep production feedback auditable: a successful customer acknowledgement
   is valid only after its tenant/property/user association and matching audit
