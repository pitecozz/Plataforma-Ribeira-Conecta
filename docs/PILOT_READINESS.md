...
|---|---:|---|
| `PILOT_READINESS_PERCENT` | 75% | The permanent HTTPS ingress, public health/endpoint restrictions, production build and public OIDC PKCE handoff are verified. The remaining gate is a real Hamilton browser-session retest of the complete authenticated journey on the permanent origin; this cannot be substituted with a development token or synthetic identity. |
...
database: **32 passed, 2 controlled external skips**. Controlled external provider tests
