...
from ribeira_platform.epistemology import DataClassification, DecisionStatus, IngestionStatus, RuleAuthority
...
class VerticalSliceTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.property = self.app.create_property(self.tenant.id, "Sítio Ribeira", {
            "type": "Polygon", "coordinates": [[[ -47.0, -24.0 ], [ -47.0, -24.1 ], [ -47.1, -24.1 ], [ -47.1, -24.0 ], [ -47.0, -24.0 ]]]
        }, "EPSG:4326")
        self.source = self.app.create_source(self.tenant.id, "sensor de umidade", "IOT", "fixture")

    def tearDown(self) -> None:
        self.store.close()

    def activate_rule(self, metric: str = "soil_moisture", threshold: float = 30.0) -> None:
        self.app.create_rule(RuleDefinition(
            id=new_id(), tenant_id=self.tenant.id, version=1, name="limiar explícito de umidade",
            authority=RuleAuthority.REGRA_AGRONOMICA, metric=metric, operator="<", threshold=threshold,
            unit="%", severity="HIGH", status="ACTIVE", approved_by="agronomist-test", valid_from="2026-09-16T00:00:00+00:00",
        ))
    ...
    def test_rule_trigger_has_evidence_alert_action_and_audit(self) -> None:
        ...
        self.app.adapters[self.source.id] = SyntheticFixtureAdapter([{
            "metric": "soil_moisture", "value": 27.4, "unit": "%",
            "observation_timestamp": "2026-09-16T13:42:11-03:00", "quality_flag": "VALID",
            "dataset_version": "fixture-v1", "crs": "EPSG:4326",
        }])
    ...
    def test_null_does_not_become_zero_or_false(self) -> None:
        self.activate_rule()
        self.app.adapters[self.source.id] = SyntheticFixtureAdapter([{
            "metric": "soil_moisture", "value": None, "unit": "%",
            "observation_timestamp": "2026-09-16T13:42:11-03:00", "quality_flag": "UNKNOWN",
        }])
        self.app.ingest(self.tenant.id, self.property.id, self.source.id)
        observations = self.store.observations_for_property(self.tenant.id, self.property.id)
    ...
    def test_conflicting_sources_are_not_resolved_silently(self) -> None:
        self.activate_rule()
        second = self.app.create_source(self.tenant.id, "sensor concorrente", "IOT", "fixture")
        ...
        for source, value in ((self.source, 27.0), (second, 45.0)):
            self.app.adapters[source.id] = SyntheticFixtureAdapter([{
                "metric": "soil_moisture", "value": value, "unit": "%", "observation_timestamp": timestamp,
                "quality_flag": "VALID",
            }])
