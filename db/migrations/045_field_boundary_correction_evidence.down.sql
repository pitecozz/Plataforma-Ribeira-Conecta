...
BEGIN
  IF EXISTS (
    SELECT 1 FROM evidence WHERE evidence_type = 'FIELD_BOUNDARY_CORRECTION'
  ) THEN
    RAISE EXCEPTION 'cannot rollback field boundary correction evidence while records exist';
  END IF;
END;
$$;

DROP INDEX field_context_boundary_version_tenant_id_uq;
