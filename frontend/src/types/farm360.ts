...
export interface PropertyCreate { name: string; geometry_geojson: Geometry; geometry_crs: string; boundary_source: string; classification: "MANUAL_CONFIRMED" | "UNKNOWN"; }
export interface SearchResult { search: { id: string; status: string; selected_scene_id: string | null; candidate_count: number; }; candidates: Array<{ scene_id: string; selected: boolean; rejection_reason: string | null; rank: number; }>; evidence_ids: string[]; }
export interface ProcessingJob { id: string; job_type: string; status: "QUEUED" | "RUNNING" | "SUCCEEDED" | "FAILED" | "BLOCKED" | "CANCELLED"; failure_code: string | null; failure_reason: string | null; output_product_id: string | null; attempt: number; max_attempts: number; heartbeat_at: string | null; created_at: string; started_at: string | null; finished_at: string | null; }
export interface BoundaryImport { id: string; tenant_id: string; property_id: string; original_filename: string; original_format: "GEOJSON"; file_size_bytes: number; file_sha256: string; original_crs: string | null; detected_crs: string | null; target_crs: string | null; geometry_checksum: string | null; geometry_geojson?: Geometry | null; boundary_source: string; classification: string; warnings: string[]; status: "NEEDS_REVIEW" | "APPROVED" | "REJECTED" | "FAILED"; created_by: string; expected_property_checksum: string | null; created_at: string; reviewed_by: string | null; review_reason: string | null; approved_boundary_version: number | null; reviewed_at: string | null; }
export interface BoundaryImportPreview { import: BoundaryImport; current_boundary_checksum: string | null; current_area_hectares: string | null; imported_area_hectares: string | null; absolute_area_delta_hectares: string | null; percentage_area_delta: string | null; current_geometry: Geometry | null; imported_geometry: Geometry | null; }
